        ...
        **kwargs
        ...
        try:
            ...
            try:
                ...
                if data is None:
                    return {"error": "API returned null JSON response", "status_code": response.status_code}
                return data
            except json.JSONDecodeError:
                # Fallback: try multipart/mixed parsing
                text = response.text
                if not text:
                    return {"error": "API returned empty response", "status_code": response.status_code}
