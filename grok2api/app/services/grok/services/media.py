...
class VideoService:
    ...
    async def completions(
        ...
        # 跨 Token 重试循环
        tried_tokens = set()
        max_token_retries = int(get_config("retry.max_retry", 3))
        last_error = None
        
        for attempt in range(max_token_retries):
            # 使用智能路由选择 token（根据视频需求与候选池）
            token_info = None
            # We must filter pool candidates manually if we want to exclude tired tokens,
            # but token_mgr.get_token_for_video doesn't support 'exclude' directly yet.
            # Instead, we will get it and check if it's tried. If it is, we would need a different logic,
            # but for now we rely on mark_rate_limited taking it out of rotation.
            ...
            try:
                # 处理图片附件 (如果同一个prompt重试，每次都需要上传吗？安全起见每次都传)
                image_url = None
                if attachments:
