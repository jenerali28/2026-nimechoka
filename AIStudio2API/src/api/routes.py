...
async def list_models(
    ...
    if parsed_model_list:
        final_model_list = [
            m for m in parsed_model_list if m.get("id") not in excluded_model_ids
        ]
...
async def chat_completions(
    ...
    logger.info(
        f"[{req_id}] 📨 收到 /v1/chat/completions 请求 (Stream={request.stream})"
    )
    launch_mode = os.environ.get("LAUNCH_MODE", "unknown")
    browser_page_critical = launch_mode != "direct_debug_no_browser"
    service_unavailable = (
            ...
            if item.get("req_id") == req_id:
                ...
                if (future := item.get("result_future")) and (not future.done()):
                    future.set_exception(
                        HTTPException(
                            status_code=499, detail=f"[{req_id}] Request cancelled."
                        )
                    )
                found = True
...
async def cancel_request(
    ...
    if request_manager.cancel_request(req_id):
        ...
        return JSONResponse(
            content={
                "success": True,
                "message": f"Active request {req_id} marked as cancelled.",
                "type": "active_request",
            }
        )
    if await cancel_queued_request(req_id, request_queue, logger):
        return JSONResponse(
            content={
                "success": True,
                "message": f"Queued request {req_id} marked as cancelled.",
                "type": "queued_request",
            }
        )
    else:
        return JSONResponse(
            ...
            content={
                "success": False,
                "message": f"Request {req_id} not found in queue or active requests.",
                "type": "not_found",
            },
        )
...
async def get_queue_status(
    ...
    return JSONResponse(
        content={
            ...
            "active_items": sorted(
                active_requests, key=lambda x: x.get("duration", 0), reverse=True
            ),
        }
    )
...
async def generate_image(
    ...
    req_id = "".join(random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=7))
    logger.info(
        f"[{req_id}] 🎨 收到 Imagen 请求 | Model: {request.model} | Count: {request.number_of_images}"
    )
...
async def generate_video(
    ...
    req_id = "".join(random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=7))
    logger.info(
        f"[{req_id}] 🎬 收到 Veo 请求 | Model: {request.model} | Duration: {request.duration_seconds}s"
    )
    ...
    try:
        ...
        if isinstance(request.image, dict) and "imageBytes" in request.image:
            image_bytes = base64.b64decode(request.image["imageBytes"])
            image_mime_type = request.image.get("mimeType", "image/png")
        elif isinstance(request.image, str) and request.image:
            image_payload = request.image.split(",", 1)[-1]
...
async def generate_nano_content(
    request: NanoRequest,
    http_request: Request,
    logger: logging.Logger = Depends(get_logger),
    server_state: Dict[str, Any] = Depends(get_server_state),
    ...
    try:
        ...
        if contents is None and request.prompt:
            prompt = request.prompt
            if isinstance(request.image, dict):
                image_bytes = (
                    base64.b64decode(request.image.get("imageBytes", ""))
                    if request.image.get("imageBytes")
                    else None
                )
                image_mime_type = request.image.get("mimeType", "image/png")
            elif isinstance(request.image, str) and request.image:
        ...
        aspect_ratio = gen_config.get("aspectRatio", request.aspect_ratio)
        ...
        raise HTTPException(
            status_code=500, detail=f"[{req_id}] Nano Banana 处理错误: {e}"
        )
