...
app = Flask(__name__)
ENABLED_MODELS = {'gemini-2.5-pro-preview-05-06', 'gemini-2.5-flash-preview-04-17', 'gemini-2.0-flash', 'gemini-2.0-flash-lite', 'gemini-1.5-pro', 'gemini-1.5-flash', 'gemini-1.5-flash-8b'}
API_URL = ''
...
def tags_endpoint():
    ...
    for model_name in ENABLED_MODELS:
        ...
        if 'llama' in model_name:
            ...
            parameter_size = 'unknown'
            quantization_level = 'unknown'
        models.append({'name': model_name, 'model': model_name, 'modified_at': datetime.now(UTC).strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'size': size, 'digest': str(uuid.uuid4()), 'details': {'parent_model': '', 'format': format, 'family': family, 'families': [family], 'parameter_size': parameter_size, 'quantization_level': quantization_level}})
...
def generate_ollama_mock_response(prompt: str, model: str) -> Dict[str, Any]:
    response_content = OLLAMA_MOCK_RESPONSES.get(prompt, f'Echo: {prompt} (这是来自模拟 Ollama 服务器的响应。)')
    return {'model': model, 'created_at': datetime.now(UTC).strftime('%Y-%m-%dT%H:%M:%SZ'), 'message': {'role': 'assistant', 'content': response_content}, 'done': True, 'total_duration': 123456789, 'load_duration': 1234567, 'prompt_eval_count': 10, 'prompt_eval_duration': 2345678, 'eval_count': 20, 'eval_duration': 3456789}
...
def print_request_params(data: Dict[str, Any], endpoint: str) -> None:
    ...
    params_str = {'端点': endpoint, '模型': model, '温度': temperature, '流式输出': stream, '消息数量': len(data.get('messages', [])), '消息预览': messages_info}
...
def ollama_chat_endpoint():
    try:
        ...
        api_request = {'model': model, 'messages': messages, 'stream': False, 'temperature': data.get('temperature', 0.7)}
...
def main():
    ...
    parser.add_argument('--main-server-port', type=int, default=DEFAULT_MAIN_SERVER_PORT, help=f'Port of the main AI Studio Proxy server (default: {DEFAULT_MAIN_SERVER_PORT})')
